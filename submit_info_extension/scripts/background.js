...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  if (changeInfo.url) {
    if (domains.some(domain => changeInfo.url.includes(domain))) {
      chrome.action.setPopup({ popup: '../html/index_state.html', tabId: tab.id });
    } else {
      chrome.action.setPopup({ popup: '../html/sleep_state.html', tabId: tab.id });
    }
  }
});
...
chrome.tabs.onActivated.addListener(async () => {
    ...
    if (tab && domains.some(domain => tab.url.includes(domain))) {
      chrome.action.setPopup({ popup: '../html/index_state.html' });
    } else {
      chrome.action.setPopup({ popup: '../html/sleep_state.html' });
    }
  });
